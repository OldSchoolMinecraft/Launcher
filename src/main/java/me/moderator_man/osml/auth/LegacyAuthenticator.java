...
public class LegacyAuthenticator
{
    ...
    public void mojangAuth(String email, String password, String clientToken)
    {
        ...
        {
            hasError = true;
            errorMessage = "Authentication failed: " + response.getString("error");
    ...
    public void hydraAuth(String email, String password)
    {
        if (Main.config.clientToken == null || Main.config.clientToken.length() != 32)
        {
            hydraAuth(email, password, generateClientToken());
        } else {
            hydraAuth(email, password, Main.config.clientToken);
        }
    }

    public void hydraAuth(String email, String password, String clientToken)
    {
        JSONObject request = new JSONObject();
        JSONObject agent = new JSONObject();
        agent.put("name", "Minecraft");
